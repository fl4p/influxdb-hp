...
date::sys_time<std::chrono::milliseconds> batchTime0(date::sys_time<std::chrono::milliseconds> &&tp, std::chrono::milliseconds &batchTime) {
...
namespace influxdb {
    ...
    client::fetchResult client::fetch(const std::string &sql, const std::array<std::string, 2> &&timeRange,
        ...
        int batches = std::ceil(milliseconds(t1 - t0).count() / (float) milliseconds(batchTime).count());

        auto fsql = sql;
        for(auto &arg : args) {
            util::replace(fsql, "?", "'" + arg + "'");
        }
    ...
    std::future<void> client::queryRaw(const std::string &sql, std::function<void(const char *, size_t)> &&callback) {
       // std::cout << sql << std::endl;
        ...
        r->Execute(std::function<void(const t_resp &response)>([result_promise, callback](const t_resp &response) {
            auto hc = response->http_code();
    ...
    std::vector<std::string> client::queryTags(const std::string &sql) {
        Document d;
        queryRaw(sql, [&](const char *body, size_t len) {
        ...
        for (int i = 0; i < series.Size(); i++) {
            tags.push_back(series[i]["tags"].MemberBegin()->value.GetString());
        }
